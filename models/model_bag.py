...
q_list_bag = """SELECT p.id, op.id AS opt_id, op.products_id, p.title, b.quantity, s.size, op.price, 
    op.discount, c.color, c.color_name, ctg.category_name, img.url_image
    ...
    INNER JOIN LATERAL (SELECT option_id, url_image FROM products_images LIMIT 1) AS img ON img.option_id = op.id
