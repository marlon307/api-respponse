q_insert_address = """INSERT INTO user_address (user_id, name_delivery, city, district, state, zipcode, street, number_home) 
    VALUES (
    (SELECT id FROM user WHERE id_user = %(user_id)s LIMIT 1),
    %(name_delivery)s, %(city)s, %(district)s, %(state)s, %(zipcode)s, %(street)s, %(number_home)s)"""
