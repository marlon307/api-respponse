...
q_login_user = (
    "SELECT id_user, name, email, password, admin FROM user WHERE (email) = (%(email)s)"
)

q_request_update_token = (
    "UPDATE user SET user_token = %(key)s WHERE (email) = %(email)s"
)
...
q_select_info_user = "SELECT name, email, birthday, cpf_cnpj, gender_id, tel, cel FROM user WHERE id_user = %(user_id)s"
